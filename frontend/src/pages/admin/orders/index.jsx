...
const Orders = () => {
    ...
    useEffect(() => {
        const isDetail =
            pathSegments.includes('add-new-product') === false &&
            (pathSegments.some(seg => seg.startsWith('product=')) ||
                (pathSegments.includes('products') &&
                    /^\d+$/.test(pathSegments[pathSegments.length - 1])));

        setIsProductDetail(isDetail);
        setIsAddNewProduct(pathSegments.includes('add-new-product'));
    }, [location]);
